...
export default function ProjectListRow({
  ...
  return (
    <Link to={`/projetos/${project.slug}`} onMouseEnter={onEnter} onMouseLeave={onLeave}
      className="group relative flex flex-col items-start gap-1 border-b border-line py-7 pl-1.5 transition-[padding] duration-300 hover:pl-7 sm:flex-row sm:items-baseline sm:justify-between sm:gap-5">
      <span className="pointer-events-none absolute left-[-4px] top-9 hidden -translate-y-1/2 translate-x-[-10px] text-2xl text-acc opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100 sm:block">→</span>
